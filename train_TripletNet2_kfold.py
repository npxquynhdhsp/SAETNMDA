...
def get_pair(args, fold, loop_i): #get pair indexes from y_4loai_fold
    ...
    if args.cb == 1:
        ...
        #-
        # train_pair_list = np.genfromtxt(args.fi_train_test + 'L' + str(loop_i) + '/train_pairs_1a1d_fold' + str(fold+1) + '.csv',
        #                                 delimiter=',').astype(int)
        # train_pair_lbl = np.genfromtxt(args.fi_train_test + 'L' + str(loop_i) + '/ytrain_1a1d_fold' + str(fold+1) + '.csv').astype(int).reshape(-1)
