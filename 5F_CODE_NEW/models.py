...
def get_sample_triplet1(args, ix,train_or_test, loop_i): #train_or_test = [0, 1] (train) / [3, 4] (test)
    ...
    if args.cb == 1:
        if train_or_test == [0, 1]:
            ...
            #-
            # train_pair_list = np.genfromtxt(args.fi_train_test + 'L' + str(loop_i) + '/train_pairs_1a1d_fold' + str(ix) + '.csv',
            #                             delimiter=',').astype(int)
            # train_pair_lbl = np.genfromtxt(args.fi_train_test + 'L' + str(loop_i) + '/ytrain_1a1d_fold' + str(ix) + '.csv').astype(int).reshape(-1)
